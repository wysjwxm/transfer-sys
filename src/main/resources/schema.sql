...
-- 转账表（transfer）：整笔一行、成功才落库（路线甲，TODO M0.3）
-- 命名即"这笔转账"的业务记录（非纯日志），后续承载状态流转/查询/冲正锚点。
...
CREATE TABLE IF NOT EXISTS transfer (
    ...
    create_time  DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    CONSTRAINT uk_transfer_txn_no  UNIQUE (txn_no),
    CONSTRAINT uk_transfer_request UNIQUE (request_no)
);
